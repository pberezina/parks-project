...
colnames(raw)<-c("year","rank","city","med park size","park size score1","park size pct", "park size score2",
                 "park access","park access socre","pakr investment","park investment score",
...
## Tried to solve by renaming the column and reorganizing the presentation slide, not work well
...
.center[
```{r remove, warning = FALSE, message=FALSE, echo=FALSE, fig.width=10,fig.height=6}
aggr(raw[,12:28], cex.axis=0.8)
raw <- select(raw, -c("city dup", "park benches", "splashground", "splashground score", "restroom score", "restroom"))
```
]
